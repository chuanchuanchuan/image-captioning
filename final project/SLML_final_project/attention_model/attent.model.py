...
"""
#captions
f = open("cap_flickr30k.json",encoding='utf-8')
captions = json.load(f)
caption = []
for v in captions:
    caption.append(v[0]['caption'])
print(len(caption))
"""
"""
f1 = open("dic_flickr30k.json",encoding='utf-8')
dic = json.load(f1)
print(dic.keys())
print(dic['images'])
#id split:train set and test set file_path
print(dic['ix_to_word'])
print(dic['wtod'])
#append '.'
"""

#data = np.load('178045.npz')
#print(data['feat'].shape)

class Caption_Generator():
    def init_weight(self, dim_in, dim_out, name=None, stddev=1.0):  # initial wieghs
        return tf.Variable(tf.truncated_normal([dim_in, dim_out], stddev=stddev / math.sqrt(float(dim_in))), name=name)

    def init_bias(self, dim_out, name=None):  # initial bias
        return tf.Variable(tf.zeros([dim_out]), name=name)

    def __init__(self, n_words, dim_embed, dim_ctx, dim_hidden, n_lstm_steps, batch_size=200, ctx_shape=[256, 2048],
                 bias_init_vector=None):
        self.n_words = n_words  # n words
        self.dim_embed = dim_embed  # word_emb dim
        self.dim_ctx = dim_ctx  # context dim
        self.dim_hidden = dim_hidden  # hidden dim
        self.ctx_shape = ctx_shape  # context shape
        self.n_lstm_steps = n_lstm_steps  # lstm steps
        ...
        self.init_memory_b = self.init_bias(dim_hidden, name='init_memory_b')  # gotcha
        ...
        self.lstm_b = self.init_bias(dim_hidden * 4, name='lstm_b')  # W U b gotcha

        self.image_encode_W = self.init_weight(dim_ctx, dim_hidden * 4, name='image_encode_W')  # Z gotcha
        ...
        self.att_b = self.init_bias(1, name='att_b')  # attention model
        ...
        self.decode_word_W = self.init_weight(dim_embed, n_words, name='decode_word_W')  # I don't know what's
    ...
    def get_initial_lstm(self, mean_context):  # gotcha
        initial_hidden = tf.nn.tanh(
            tf.matmul(mean_context, self.init_hidden_W) + self.init_hidden_b)  # initial mean_context
        initial_memory = tf.nn.tanh(tf.matmul(mean_context, self.init_memory_W) + self.init_memory_b)

        return initial_hidden, initial_memory

    def build_model(self):
        context = tf.placeholder("float32",
                                 [self.batch_size, self.ctx_shape[0], self.ctx_shape[1]])  # batch_size images
        sentence = tf.placeholder("int32", [self.batch_size, self.n_lstm_steps])  # batch_size sentence
        mask = tf.placeholder("float32", [self.batch_size, self.n_lstm_steps])

        h, c = self.get_initial_lstm(tf.reduce_mean(context, 1))  # initial
        ...
        for ind in range(self.n_lstm_steps):
            ...
            if ind == 0:
                ...
                with tf.device("/cpu:0"):
                    word_emb = tf.nn.embedding_lookup(self.Wemb, sentence[:, ind - 1])  # E*y(t-1)
            ...
            context_encode = context_encode + \
                             ...
                             self.pre_att_b  # attention function

            context_encode = tf.nn.tanh(context_encode)  # attention function
            ...
            alpha = tf.nn.softmax(alpha)  # alpha

            weighted_context = tf.reduce_sum(context * tf.expand_dims(alpha, 2), 1)  # weighted_context
            ...
            logits = tf.nn.dropout(logits, 0.5)  # What's this
...
def train(pretrained_model_path=None): 
    ...
    for i in range(0,len(dics),1):
        if dics[i]['split'] == 'train':
            dic.append(dics[i])
            caption.append(captions[i][0]['caption'])
            cou = cou + 1
            if cou == 100:
                break
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(caption)
    print("done!!!!")
    learning_rate = 0.1
    n_words = len(wordtoix)   
    path = '/home/lbc/lbc/fea_att/'
    maxlen = np.max(list(map(lambda x: len(x), caption)))
    ...
    for epoch in range(n_epochs):
        ...
        for start, end in zip( \
            ...
            print(i)
            i = i + 1
            current_dic = dic[start:end]
            feats = []
            for v in current_dic:
                image = v['id']
                image_id = path + str(image) + '.npz'
                data = np.load(image_id)
                feat = data['feat']
                feat = feat.tolist()
                feats.append(feat)
            current_feats = np.array(feats)
            current_feats = current_feats.reshape(-1, ctx_shape[1], ctx_shape[0]).swapaxes(1, 2)

            current_captions = caption[start:end]
            current_caption_ind = list(map(lambda cap: [wordtoix[word] for word in cap[:-1] if word in wordtoix],current_captions))#debug
            current_caption_matrix = sequence.pad_sequences(sequences=current_caption_ind,padding='post',maxlen=maxlen+1)#debug
...
def test(test_feat='4801369809.npz', model_path='models/model-99', maxlen=20):
    f = open("cap_flickr30k.json", encoding='utf-8')
    captions = json.load(f)
    caption = []
    f1 = open("dic_flickr30k.json",encoding='utf-8')
    ...
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(caption)
    n_words = len(wordtoix)
    data = np.load('/home/lbc/lbc/fea_att/4801369809.npz')
    feat = data['feat']
